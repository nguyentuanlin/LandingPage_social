        ...
        <div className="grid md:grid-cols-1 gap-8 max-w-3xl mx-auto">
          {plans.map((plan, idx) => (
            <motion.div
              ...
              className={`relative p-8 md:p-10 rounded-3xl backdrop-blur-xl border transition-all pricing-card ${
                plan.highlighted
                  ? 'pricing-card--highlight bg-white/5 border-white/15 shadow-xl hover:shadow-2xl hover:border-cyan-400/40'
                  : 'bg-white/5 border-white/10 hover:border-white/20 shadow-lg'
              }`}
            >
              {plan.highlighted && (
                <div className="pricing-badge absolute -top-4 left-6 px-3 py-1 rounded-full bg-white/10 border border-white/25 text-xs font-semibold text-white/90 backdrop-blur">
                  Phổ biến nhất
                </div>
              )}
              ...
              <a
                ...
                className={`pricing-cta w-full mt-3 py-3 rounded-xl font-bold transition-all ${
                  plan.highlighted
